...
def generate_password():  # generate password
    if len(password_string) == 0: #проверка списка по длинне чтобы было из чего собирать пароль,и обрабатывает пустое значение
        ...
        table_name.delete(0,'end')  #это должно было очистить содержимое окна,перед выводом в него сгеенерированного результата
        string_pass = ''
...
Label(text="Приветствую тебя друг! Давай соберем тебе стойкий пароль!",font=('Arial',14)).grid(row=0,column=0,columnspan=5, pady=10)
